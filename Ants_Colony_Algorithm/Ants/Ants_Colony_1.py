...
def initialization():
    ...
    # 初始化城市间路径信息素矩阵
    phernomones = np.array([1 for i in range(30)] * 30).reshape(30, 30)
    # 初始化蚂蚁群
    for i in range(Np):
...
def decision(city_now, jk, alp, beta):
    ...
    for item in jk:
        ...
        tmp_pheno.append(phernomones[city_now][item])
...
def iteration():
    ...
    while iters > 0:
        ...
        info_q[G - iters] = tmp
        # 信息素挥发
        phernomones = (1 - rho) * phernomones
        # 按照蚁周模型更新信息素
        for i in range(Np):
            for j in range(30):
                m, n = ants[i]['tabu'][j - 1], ants[i]['tabu'][j]
                # 蚁量模型
                # phernomones[m][n] += Q/cities_dis[m][n]
                # phernomones[n][m] += Q/cities_dis[m][n]
                # 蚁密模型
                '''phernomones[m][n] += Q
                phernomones[n][m] += Q'''
                # 蚁周模型
                phernomones[m][n] += Q / ants[i]['L']
                phernomones[n][m] += Q / ants[i]['L']
        # 清空蚂蚁禁忌表并随机开始城市
        for i in range(Np):
            reset_ant(ants[i])
...
def showresult_1(it_x, it_y):
    ...
    plt.xlabel('Iterations')
    plt.ylabel('Distance of shortest path')
    plt.title('iterations={}  alpha={} beta={} Q={} rho={} min_dist={}'.format(G, alpha, beta, Q, rho, best))
    plt.plot(it_x[0], it_y[0], 'b', label='Np=10')
    plt.plot(it_x[1], it_y[1], 'r', label='NP=50')
    plt.plot(it_x[2], it_y[2], 'g', label='Np=100')
